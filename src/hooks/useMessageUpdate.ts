...
export default function useMessageUpdate(messages: StoredMessage[] | undefined, latestMessage: StoredMessage | undefined, deleteMessage: string | undefined) {
    ...
    useEffect(() => {
        ...
        if (latestMessage &&  prevLatest.current !== latestMessage.id) {
            ...
            prevLatest.current = latestMessage.id
        }
        //if not delete message, this means that in the case the first if statement is not passed, this will only run when compnent mounts
    ...
    }, [messages, latestMessage, deleteMessage])
    return [showMessages]
